...
import { GetRandomFeedBackAction } from "~/src/features/feedback/get/getRandomFeedback.action";
    ...
    <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden py-20">
      <Marquee pauseOnHover className="[--duration:20s]">
        {firstRow.map((feedback) => (
          <ReviewCard key={feedback.id} img="https://avatar.vercel.sh/emilyjohnson" name={feedback.user?.name} message={feedback.message} createAt={feedback.createdAt} />
        ))}
      </Marquee>
